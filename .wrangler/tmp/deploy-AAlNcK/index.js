...
var app = new Hono2();
async function readTextFile(env3, filePath) {
  try {
    const file = await env3.ROOT.fetch(new Request(`https://placeholder/${filePath}`));
    if (!file.ok) {
      throw new Error(`File not found: ${filePath}`);
    }
    return await file.text();
  } catch (error3) {
    throw new Error(`Error reading file: ${filePath}, ${error3}`);
  }
}
__name(readTextFile, "readTextFile");
async function readBinaryFile(env3, filePath) {
  try {
    const file = await env3.ROOT.fetch(new Request(`https://placeholder/${filePath}`));
    if (!file.ok) {
      throw new Error(`File not found: ${filePath}`);
...
__name(readBinaryFile, "readBinaryFile");
async function fileExists(env3, filePath) {
  try {
    const file = await env3.ROOT.fetch(new Request(`https://placeholder/${filePath}`));
    return file.ok;
  } catch (error3) {
...
__name(getAllMetadata, "getAllMetadata");
app.get("/.well-known/:file", async (c) => {
  ...
  if (await fileExists(c.env, filePath)) {
    ...
    if (c.req.header("accept")?.includes("html")) {
      const html2 = await readTextFile(c.env, "404.html");
      return c.html(html2);
    }
...
app.get("/blogs", async (c) => {
  ...
  const html2 = template.replaceAll("[blogs.view]", blogs);
  return c.html(html2);
});
app.get("/:doc", async (c) => {
